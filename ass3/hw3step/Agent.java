 ...
 *  	  If we can not find the path, do the next step following the Strategy.
 		3.In the A Star algorithm, it relates with stone/raft, we will also try to use many simulate states
 		  to represet these status, once we need to use them, we update the status and if we used all and still can't
 		  find, then we give up this thread.
 		4. In terms of the vivitedview object, we will update its status after taking any actions.
 *  
 *  3. Data Structures used:
 *  	1.priority queue: Store and sort the data in A* search
 *  	2.Arraylist : Store the data 
 *  	3.HashMap: Store and data
 *		4.2d array: Stores the map information
 *      5.State: stores the state after a move
 *
 *   4. The Agent sees 14,000,605 versions of the future, and in only one of those timelines he finds
 *		the treasure.
